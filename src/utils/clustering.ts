...
export function calculateSilhouetteScore(data: number[][], clusters: number[]): number {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const aI = sameClusterPoints.length > 0 
      ? sameClusterPoints.reduce((sum, point) => sum + mlDistance.euclidean(data[i], point), 0) / sameClusterPoints.length
      : 0;
    ...
    for (const cluster of otherClusters) {
      ...
      if (otherClusterPoints.length > 0) {
        const avgDistance = otherClusterPoints.reduce((sum, point) => sum + mlDistance.euclidean(data[i], point), 0) / otherClusterPoints.length;
